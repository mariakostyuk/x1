...
class Geo {
    ...
    public function getCity($geoData) {
	if(isset($geoData['city']) && isset($geoData['region']))
	...
	$v=filter_input(INPUT_COOKIE, 'geobase');
        if ($cookie && $v) {		
		if($v!="Not found")
        ...
        if (!empty($data)) {
            	...
            	setcookie('geobase', "Not found", time() + 3600 * 24 * 7, '/'); //устанавливаем куки на неделю
